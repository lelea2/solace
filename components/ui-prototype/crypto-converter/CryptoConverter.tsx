...
type PriceDirection = "up" | "down" | "same" | null;
...
export default function CryptoConverter() {
  ...
  return (
    <main className={styles.container}>
      <h1 className={styles.title}>WUC Crypto Converter</h1>
      ...
      <section className={styles.result}>
        {isLoading && <p className={styles.loading}>Refreshing price...</p>}
...
function PriceChange({ direction }: { direction: PriceDirection }) {
  if (direction == null || direction === "same") {
    return <span className={styles.priceChangeNeutral}>—</span>;
  }

  if (direction === "up") {
    return <span className={styles.priceChangeUp}>↑</span>;
  }

  return <span className={styles.priceChangeDown}>↓</span>;
}